...
class SpartusOrchestrator:
    ...
    def _on_stop_trading(self) -> None:
        """Pause AI trading -- open MT5 positions are NOT closed.

        Stops the AI from making new decisions. Any open position
        remains active on MT5 with its SL/TP intact. Use Emergency
        Stop if you want to force-close positions immediately.
        """
        if self._executor is None:
            return
        self._executor.pause_trading()
        self._dashboard.set_trading_state(DashState.STOPPED)
        self._add_alert("INFO", "AI trading paused — open positions remain active on MT5")
        log.info("User paused AI trading (positions left open on MT5)")
