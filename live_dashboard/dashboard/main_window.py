...
class LiveDashboard(QMainWindow):
    ...
    def closeEvent(self, event) -> None:
        ...
        Stops the update timer and accepts the close event.
        MT5 positions are intentionally NOT closed here — the broker's
        own SL/TP orders remain active on the server.
        Use Emergency Stop if you want to force-close everything first.
        """
        self._stop_update_timer()
        log.info("Dashboard window closed — MT5 positions (if any) left intact on broker")
        event.accept()
